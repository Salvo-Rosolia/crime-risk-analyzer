...
const data: AnalyzeResponse = {
  ...
  poi: [
    { id: '1', name: 'A', terminus_class: 'x', lat: 0, lon: 0, confidence: 'confermato', sparql_path: null, terminus_label_it: '', terminus_label_en: '' },
    { id: '2', name: 'B', terminus_class: 'x', lat: 0, lon: 0, confidence: 'confermato', sparql_path: null, terminus_label_it: '', terminus_label_en: '' },
    { id: '3', name: 'C', terminus_class: 'x', lat: 0, lon: 0, confidence: 'plausibile', sparql_path: null, terminus_label_it: '', terminus_label_en: '' },
    { id: '4', name: 'D', terminus_class: 'x', lat: 0, lon: 0, confidence: 'speculativo', sparql_path: null, terminus_label_it: '', terminus_label_en: '' },
  ],
  risk_models: [
    { poi: 'A', risks: [{ hazard: 'h1', confidence: 'confermato', tag: 'ONTOLOGIA', hazard_label_it: '', hazard_label_en: '' }] },
  ],
  narrativa: '',
...
describe('HeaderControlsComponent', () => {
  ...
  function setup(inputs: { data?: AnalyzeResponse | null; mode?: 'completo' | 'base'; filter?: 'confermato' | 'plausibile' | 'speculativo' | null; loading?: boolean } = {}): void {
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({ imports: [HeaderControlsComponent] }).compileComponents();
  });
  ...
  it('click su "Base" emette toggleMode(\'base\')', () => {
    ...
    const buttons: HTMLButtonElement[] = Array.from(fixture.nativeElement.querySelectorAll('.cra-mode-btn'));
  ...
  it('con dati in modalità completo mostra il badge Copertura qualitativo (via deriveCoverage/coverageBadgeText, somma confidence_summary = 5, non il conteggio POI)', () => {
    ...
    expect(fixture.nativeElement.textContent).toContain('Copertura 5 rischi · 1 ancorati a ontologia');
  });
  ...
  it('click su un chip emette setFilter; riclic sul chip attivo emette clearFilter', () => {
    ...
    const chips: HTMLButtonElement[] = Array.from(fixture.nativeElement.querySelectorAll('.cra-chip'));
    chips[0].click();
    expect(setSpy).toHaveBeenCalledWith('confermato');

    fixture.componentRef.setInput('filter', 'confermato');
    fixture.detectChanges();
    const chipsAfter: HTMLButtonElement[] = Array.from(fixture.nativeElement.querySelectorAll('.cra-chip'));
    chipsAfter[0].click();
