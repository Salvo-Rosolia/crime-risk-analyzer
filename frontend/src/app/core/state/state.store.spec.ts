...
describe('StateStore', () => {
  ...
  it('startAnalysis success → LOAD_SUCCESS con i dati', async () => {
    ...
    await store.startAnalysis('Colosseo', null);
    expect(api.analyze).toHaveBeenCalledWith('Colosseo', null);
  ...
  it('startAnalysis con domanda passa la domanda all\'api', async () => {
    api.analyze.mockResolvedValue(data);
    await store.startAnalysis('Roma', 'di sera?');
    expect(api.analyze).toHaveBeenCalledWith('Roma', 'di sera?');
  });
